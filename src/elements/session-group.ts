    ...
    return html`
      <section class="overflow-hidden rounded-[20px] border border-border bg-bg-light shadow-card">
        <header class="flex min-w-0 items-center gap-2.5 border-b border-border bg-bg-light/80 px-4 py-3">
          ...
          <span class="rounded-lg bg-bg px-2 py-1 font-mono text-[9px] font-bold text-describe">${sessions.length}</span>
        </header>
        <div>
          ${visibleSessions.map(
            (session) => html`
        ...
        <footer v-if=${sessions.length > 5} class="border-t border-border/70 bg-bg-light/40">
          <button
            type="button"
            class="flex h-10 w-full cursor-pointer items-center justify-center gap-1.5 border-0 bg-transparent text-xs font-semibold text-describe transition-colors hover:text-highlight active:bg-bg-hover"
