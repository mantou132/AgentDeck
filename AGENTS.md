...
- `src/elements/`：composer 管理输入和附件，session-timeline / process-detail 展示消息与过程，attachment / attachment-preview 展示附件；file-browser 通过 `agent_file_read` 浏览远端文件，消息链接由 `navigation.ts` 分流到 Tap Browser 或文件 Stack；sheet.ts 封装弹层及内部 sheet-layer，通过 tap-reflect 映射到 body 并保留样式作用域；carousel 提供通用 CSS Scroll Snap 分页，relay-guide 提供三步指南图文；其余为目录、会话分组、权限和品牌组件。
