...
- `src/elements/`：composer 管理输入和附件，session-timeline / process-detail 展示消息与过程，attachment / attachment-preview 展示附件；sheet.ts 封装弹层及内部 sheet-layer，通过 tap-reflect 映射到 body 并保留样式作用域；其余为目录、会话分组、权限和品牌组件。
