...
const { AGENT_PANEL_STATE_KEY, agentSessionKey } = await import('../shared/agent-session-store.js');
...
describe('agent panel runtime', () => {
  ...
  it('shows a draft prompt immediately and keeps responding while the ACP session is created', async () => {
    ...
    const api = {
      ...
      prompt: (_sessionId, _agent, prompt) =>
        new Promise((resolve) => {
          askCalls.push({ prompt, resolve: (answer) => resolve({ answer }) });
        }),
    };
    let controller;
  ...
  it('keeps queued prompts after cancel instead of auto-draining them', async () => {
    ...
    const turns = createTurnController({
      ...
      api: {
        prompt: () => {
          askCount += 1;
          return new Promise((resolve) => (resolveAsk = (answer) => resolve({ answer })));
        },
        cancelPrompt: async () => {
          resolveAsk('partial');
          return true;
